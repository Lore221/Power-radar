 ...
 * field(...) постоянно показывает параметр, а text("lang.key") добавляет
 * описание только при зажатом Shift.
 */
public final class PowerRadarTooltipSettings {
        ...
        public record Line(
                ...
                // Старый формат строки без отдельного цвета выделения сохраняет один цвет для
                // всего текста.
        ...
        // Постоянные параметры предметов. Очки меняют только отображение: оценка или
        // точное значение.
        private static final Map<Target, List<Line>> INVENTORY_PARAMETERS = Map.ofEntries(
                        ...
                        Map.entry(Target.AIR_RADAR_CONTROLLER, List.of(
                                        field(InventoryField.NOMINAL_POWER),
                                        field(InventoryField.NOMINAL_VOLTAGE))),
        ...
        // В lang обрамляй выделяемые слова символами "_": "Обычный текст _выделенный
        // текст_".
        // Здесь задаются основной цвет и цвет выделения для всех простых вызовов
        // text("ключ.из.lang").
        ...
        // Заголовок добавляется без отдельного отступа: вся готовая подсказка
        // выравнивается одним проходом.
         ...
         * Индекс нужен, чтобы не менять строки, которые другой источник мог добавить в
         * список раньше.
         */
        ...
        /**
         * Добавляет произвольную lang-строку и выделяет другим цветом фрагменты между
         * символами "_".Б
